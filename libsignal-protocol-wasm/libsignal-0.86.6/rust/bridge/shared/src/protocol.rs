...
#[wasm_bindgen]
pub fn kyber_keygen_wasm() -> Result<JsValue, JsValue> {
    // Use the concrete Kyber1024 parameters — adjust if API differs
    let params = crate::kem::KeyType::Kyber1024.parameters();
    // If the parameters API exposes a `keypair()` or similar:
    match params.generate_keypair() {
        Ok((pubk, privk)) => {
            let pub_b64 = base64::engine::general_purpose::STANDARD.encode(pubk);
            let priv_b64 = base64::engine::general_purpose::STANDARD.encode(privk);
            let obj = js_sys::Object::new();
            js_sys::Reflect::set(&obj, &"public_key".into(), &JsValue::from_str(&pub_b64)).unwrap();
            js_sys::Reflect::set(&obj, &"secret_key".into(), &JsValue::from_str(&priv_b64)).unwrap();
            Ok(JsValue::from(obj))
        }
        Err(e) => Err(JsValue::from_str(&format!("kyber keygen failed: {}", e))),
    }
}
