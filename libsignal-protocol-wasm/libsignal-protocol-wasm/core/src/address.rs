...
impl Distribution<DeviceId> for Standard {
    fn sample<R: rand::Rng + ?Sized>(&self, rng: &mut R) -> DeviceId {
        // Generate integer in 1..=127
        let value: u8 = rng.gen_range(1..=MAX_VALID_DEVICE_ID);

        // Safe because it's guaranteed non-zero and <= 127
        DeviceId::new(value).expect("value is always in valid range")
    }
}
*/
