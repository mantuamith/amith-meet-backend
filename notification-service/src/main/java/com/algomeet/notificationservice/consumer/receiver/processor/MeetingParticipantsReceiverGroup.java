...
@Component
public class MeetingParticipantsReceiverGroup implements ReceiverGroupProcessor{
	@Autowired
	...
	public List<UserDto> getUserList(NotificationDto notificationDto) {
		if (!(StringUtils.hasLength(notificationDto.getReceiverGroup()))
				|| ReceiverGroup.MEETING_PARTICIPANTS != ReceiverGroup.valueOf(notificationDto.getReceiverGroup())) {
			return null;
		}
		...
		participantList.addAll(
				meetingNativeRepository.getParticipantList(notificationDto.getReceiverGroupRefId()));

		participantList.add(
				meetingNativeRepository.getMeetingHost(notificationDto.getReceiverGroupRefId()));
		
		List<UserDto> userList = null;				
		if (!CollectionUtils.isEmpty(participantList)) {
			userList = userNativeRepository.getUsersByEmailList(participantList);
		}
