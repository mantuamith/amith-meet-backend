...
public class UnpinMessageHandler {
	...
	public void handlePin(ChannelHandlerContext ctx, String id, XmppPrincipal principal, ViewManageStaxParser.ViewItem item){
		if (StringUtils.hasText(item.room)) {
			...
			pinMucMessageService.unpinMessage(UUID.fromString(groupId), UUID.fromString(item.id), UUID.fromString(principal.getUserKey()))
			...
			pinChatMessageService.unpinMessage(UUID.fromString(principal.getUserKey()), UUID.fromString(peerKey), UUID.fromString(item.id))
					.subscribeOn(DB_SCHEDULER)
					.flatMap(savedDoc -> {		
