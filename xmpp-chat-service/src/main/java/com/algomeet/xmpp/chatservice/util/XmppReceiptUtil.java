...
@Slf4j
@Component
public class XmppReceiptUtil {

    public static final String NS_RECEIPTS = "urn:xmpp:receipts";
    private static final String EL_RECEIVED = "received";
    private static final String ATTR_ID = "id";

    private final XMLInputFactory factory;

    public XmppReceiptUtil() {
    ...
    public String getAckMessageId(String xml) {
        if (xml == null || !xml.contains(NS_RECEIPTS)) {
            return null;
        }
        ...
        try {
            ...
            while (reader.hasNext()) {
                ...
                if (event == XMLStreamConstants.START_ELEMENT) {
                    if (EL_RECEIVED.equals(reader.getLocalName()) && 
                        NS_RECEIPTS.equals(reader.getNamespaceURI())) {
                        return reader.getAttributeValue(null, ATTR_ID);
                    }
                }
            }
        } catch (Exception e) {
            log.error("Failed to parse XMPP stanza for receipt detection", e);
        } finally {
            closeReader(reader);
        }
        return null;
